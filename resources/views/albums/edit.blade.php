    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.albums.update', $album->id) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="3" 
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('deskripsi', $album->deskripsi) }}</textarea>
                            @error('deskripsi')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>

                        <div x-data="{ fileName: '' }">
                            <label class="block text-sm font-medium text-gray-700">Cover Baru (Opsional)</label>
                            @if($album->cover)
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 mb-1">Cover saat ini:</p>
                                    <img src="{{ asset('storage/' . $album->cover) }}" alt="Current cover" class="w-full aspect-video object-cover rounded-md">
                                </div>
                            @endif
